...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if ($check->get_result()->num_rows>0) {
        ...
        $stmt=$conn->prepare("INSERT INTO users (full_name,email,role,status) VALUES (?,?,'caregiver','pending_password')");
        $stmt->bind_param("ss",$full_name,$email); $stmt->execute();
        sendWelcomeEmail($email,$full_name);
        $message="<div class='alert alert-success'><i class='fas fa-check-circle'></i>Caregiver added! Welcome email sent.</div>";
    }
}
?>
<!DOCTYPE html>
...
<div class="sidebar">
    ...
    <div class="sidebar-nav">
        ...
        <a href="manage_caregivers.php" class="active"><i class="fas fa-user-nurse"></i> Manage Caregivers</a>
  ...
  <div style="width:100%;max-width:520px;">
    <div class="form-card">
        <div class="form-card-header">
            <div class="form-card-header-icon"><i class="fas fa-user-nurse"></i></div>
            <h3>Add New Caregiver</h3>
            <p>Invite a caregiver to join your team</p>
        </div>
        <div class="form-card-body">
            <?php echo $message; ?>
            <div class="info-note">
                <i class="fas fa-envelope" style="color:var(--s400);flex-shrink:0;"></i>
                Caregiver will receive a welcome email with a link to set their password.
            </div>
            <form method="POST" id="caregiverForm">
                <div class="mb-4">
                    ...
                    <div class="input-icon-wrap">
                        ...
                        <input type="text" name="full_name" class="form-control" required placeholder="Enter caregiver's full name">
                    </div>
                </div>
                <div class="mb-4">
                    ...
                    <div class="input-icon-wrap">
                        ...
                        <input type="email" name="email" class="form-control" required placeholder="Enter caregiver's email address">
                    </div>
                </div>
                <div class="d-flex gap-3 mt-2">
                    <a href="manage_caregivers.php" class="btn-outline"><i class="fas fa-arrow-left"></i>Back</a>
                    <button type="submit" class="btn-save flex-grow-1"><i class="fas fa-user-nurse"></i>Add Caregiver</button>
                </div>
            </form>
        </div>
    </div>
...
document.getElementById('caregiverForm').addEventListener('submit',function(e){
    ...
    btn.innerHTML='<i class="fas fa-spinner fa-spin"></i>Adding Caregiver...';btn.disabled=true;
});
