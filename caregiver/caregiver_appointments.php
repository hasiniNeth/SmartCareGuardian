...
$pk_stmt->bind_param("i", $caregiver_user_id);
$pk_stmt->execute();
$pk_row = $pk_stmt->get_result()->fetch_assoc();
$pk_stmt->close();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_appointment'])) {
    ...
    $auth->bind_param("ii", $caregiver_user_id, $resident_id);
    $auth->execute();
    $authorized = $auth->get_result()->num_rows > 0;
    $auth->close();
    ...
    if (!$authorized) {
        ...
        if ($is_dup) {
            ...
            if ($ins->execute()) {
                ...
                $emailStmt->execute();
                $resident_email_data = $emailStmt->get_result()->fetch_assoc();
                $emailStmt->close();
                ...
                if ($resident_email_data && !empty($resident_email_data['email'])) {
                    try {
                        ...
                        $mail->Body = "
                            ...
                            <h3 style='color:#4A766E;'>Hello {$resident_email_data['full_name']},</h3>
                            <p>A new appointment has been scheduled for you by your caregiver.</p>
                            <table style='width:100%;border-collapse:collapse;margin:16px 0;'>
                                <tr><td style='padding:8px;background:#E8F5E8;font-weight:bold;width:35%;'>Title</td><td style='padding:8px;border-bottom:1px solid #eee;'>{$title}</td></tr>
                                <tr><td style='padding:8px;background:#E8F5E8;font-weight:bold;'>Date</td><td style='padding:8px;border-bottom:1px solid #eee;'>" . date('F j, Y', strtotime($appointment_date)) . "</td></tr>
                                <tr><td style='padding:8px;background:#E8F5E8;font-weight:bold;'>Time</td><td style='padding:8px;border-bottom:1px solid #eee;'>" . date('g:i A', strtotime($appointment_time)) . "</td></tr>
                                <tr><td style='padding:8px;background:#E8F5E8;font-weight:bold;'>Location</td><td style='padding:8px;'>{$location}</td></tr>
                            </table>
                            <p style='color:#888;font-size:.9rem;'>Please be ready on time. — SmartCare Guardian</p>
                            </div>
                        ";
                        $mail->send();
                    } catch (Exception $e) {
...
$res_stmt->bind_param("i", $caregiver_user_id);
$res_stmt->execute();
$assigned_residents = $res_stmt->get_result();
$res_stmt->close();
...
$today_stmt->bind_param("is", $caregiver_user_id, $today);
$today_stmt->execute();
...
$all_stmt = $conn->prepare($sql);
$all_stmt->bind_param($types, ...$params);
$all_stmt->execute();
$appointments = $all_stmt->get_result();
$all_stmt->close();
...
$counts_stmt->bind_param("i", $caregiver_user_id);
$counts_stmt->execute();
...
$res2_stmt->bind_param("i", $caregiver_user_id);
$res2_stmt->execute();
$filter_residents = $res2_stmt->get_result();
$res2_stmt->close();
...
$med_stmt->bind_param("i", $caregiver_user_id);
$med_stmt->execute();
...
<head>
    ...
    /* ═══════════════════════════════════════════════════════════
       ...
       Caregiver Appointments · Professional scale
    ═══════════════════════════════════════════════════════════ */
    :root{
...
<div class="content">
    ...
    <div class="summary-grid">
        <div class="summary-card">
            <div class="s-icon s-total"><i class="fas fa-calendar-alt"></i></div>
            <div class="s-number"><?= $counts['total'] ?? 0 ?></div>
            <div class="s-label">Total</div>
        </div>
        <div class="summary-card">
            <div class="s-icon s-scheduled"><i class="fas fa-calendar-check"></i></div>
            <div class="s-number"><?= $counts['scheduled'] ?? 0 ?></div>
            <div class="s-label">Scheduled</div>
        </div>
        <div class="summary-card">
            <div class="s-icon s-completed"><i class="fas fa-check-double"></i></div>
            <div class="s-number"><?= $counts['completed'] ?? 0 ?></div>
            <div class="s-label">Completed</div>
        </div>
        <div class="summary-card">
            <div class="s-icon s-cancelled"><i class="fas fa-calendar-xmark"></i></div>
            <div class="s-number"><?= $counts['cancelled'] ?? 0 ?></div>
            <div class="s-label">Cancelled</div>
        </div>
        <div class="summary-card">
            <div class="s-icon s-scheduled"><i class="fas fa-calendar-day"></i></div>
            <div class="s-number"><?= $today_count ?></div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="resident_id" class="form-select" required>
                            ...
                            <?php while ($res = $assigned_residents->fetch_assoc()): ?>
                                <option value="<?= $res['user_id'] ?>" <?= (isset($_POST['resident_id']) && $_POST['resident_id'] == $res['user_id']) ? 'selected' : '' ?>>
                        ...
                        <div class="appt-card <?= $ap['status'] ?>">
                            ...
                            <div class="act-btns mt-auto">
                                <form method="POST" style="display:inline;">
                                    ...
                                    <button type="submit" name="update_status" class="btn-icon btn-complete" title="Mark as Completed"><i class="fas fa-check"></i></button>
                                </form>
                                <form method="POST" style="display:inline;">
                                    <input type="hidden" name="appointment_id" value="<?= $ap['appointment_id'] ?>">
                                    <input type="hidden" name="new_status" value="cancelled">
                                    <button type="submit" name="update_status" class="btn-icon btn-cancel-ap" title="Cancel Appointment"><i class="fas fa-ban"></i></button>
                                </form>
                                <form method="POST" style="display:inline;" onsubmit="return confirm('Delete this appointment?');">
                                    <input type="hidden" name="appointment_id" value="<?= $ap['appointment_id'] ?>">
                                    <button type="submit" name="delete_appointment" class="btn-icon btn-delete" title="Delete"><i class="fas fa-trash"></i></button>
                                </form>
                            </div>
                            <?php endif; ?>
                        </div>
    ...
    <div class="table-card">
        ...
        <div class="filter-bar">
            <form method="GET" style="display:contents;">
                ...
                <div class="filter-group">
                    ...
                    <select name="filter_resident" class="form-select">
                        <option value="0">All Residents</option>
                        <?php while ($fr = $filter_residents->fetch_assoc()): ?>
                            <option value="<?= $fr['user_id'] ?>" <?= $filter_resident===(int)$fr['user_id']?'selected':'' ?>><?= htmlspecialchars($fr['full_name']) ?></option>
                        <?php endwhile; ?>
                    </select>
                </div>
                <div class="filter-group">
        ...
        <div class="table-responsive">
            <table class="table">
                <thead>
                    <tr><th>Resident</th><th>Title</th><th>Date</th><th>Time</th><th>Location</th><th>Status</th><th>Actions</th></tr>
                </thead>
                <tbody>
                        ...
                        <tr>
                            <td>
                                <div style="display:flex;align-items:center;gap:8px;">
                                    <div class="res-avatar" style="width:30px;height:30px;font-size:.8rem;"><?= strtoupper(substr($ap['resident_name'],0,1)) ?></div>
                                    <strong style="color:var(--s800);"><?= htmlspecialchars($ap['resident_name']) ?></strong>
                                </div>
                            </td>
                            <td>
                                <?= htmlspecialchars($ap['title']) ?>
                                <?php if ($ap['description']): ?><div style="font-size:11px;color:var(--st300);margin-top:1px;"><?= mb_strimwidth(htmlspecialchars($ap['description']),0,50,'…') ?></div><?php endif; ?>
                            </td>
                            <td>
                                <strong style="color:var(--s800);"><?= date('M j, Y', strtotime($ap['appointment_date'])) ?></strong>
                                <?php if ($ap['appointment_date'] === $today): ?><span class="badge badge-today">Today</span><?php endif; ?>
                            </td>
                                ...
                                <div class="act-btns">
                                    <?php if ($ap['status'] === 'scheduled'): ?>
                                        <form method="POST" style="display:inline;">
                                            <input type="hidden" name="appointment_id" value="<?= $ap['appointment_id'] ?>">
                                            <input type="hidden" name="new_status" value="completed">
                                            <button type="submit" name="update_status" class="btn-icon btn-complete" title="Mark Completed"><i class="fas fa-check"></i></button>
                                        </form>
                                        <form method="POST" style="display:inline;">
                                            <input type="hidden" name="appointment_id" value="<?= $ap['appointment_id'] ?>">
                                            <input type="hidden" name="new_status" value="cancelled">
                                            <button type="submit" name="update_status" class="btn-icon btn-cancel-ap" title="Cancel"><i class="fas fa-ban"></i></button>
                                        </form>
                                    <?php endif; ?>
                                    <form method="POST" style="display:inline;" onsubmit="return confirm('Delete this appointment?');">
                                        <input type="hidden" name="appointment_id" value="<?= $ap['appointment_id'] ?>">
                    ...
                    <?php else: ?>
                        <tr><td colspan="7">
                            <div class="empty-st"><i class="fas fa-calendar-xmark"></i><p>No appointments found<?= $filter_status !== 'all' || $filter_resident || $filter_from ? ' matching the selected filters.' : '.' ?></p></div>
                        </td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
...
<script>
document.getElementById('createForm')?.addEventListener('show.bs.collapse', () => {
    document.getElementById('collapseIcon').className = 'fas fa-chevron-up';
});
document.getElementById('createForm')?.addEventListener('hide.bs.collapse', () => {
    document.getElementById('collapseIcon').className = 'fas fa-chevron-down';
});
