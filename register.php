...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if ($check->get_result()->num_rows>0) { $error="Email already registered."; }
        ...
        $_SESSION['temp_user']=['full_name'=>$name,'email'=>$email,'password'=>$password];
        $stmt=$conn->prepare("INSERT INTO otp_verification (email,otp,otp_expiry) VALUES (?,?,?)");
        $stmt->bind_param("sss",$email,$otp,$otp_expiry);
        if ($stmt->execute()) {
            if (sendOTP($email,$otp)) { header("Location: verify_otp.php"); exit(); }
...
<div class="auth-card">
    <div class="auth-header">
        ...
        <div class="auth-header-icon"><i class="fas fa-user-plus"></i></div>
        <h2>Join SmartCare Guardian</h2>
        <p>Begin your journey to personalised elderly care</p>
    </div>
    <div class="auth-body">
        <?php if (isset($error)): ?>
        ...
        <form method="POST" id="registrationForm">
            ...
            <div class="mb-4">
                <label class="form-label"><i class="fas fa-lock" style="color:var(--s400);"></i>Password</label>
                <div class="input-icon-wrap">
                    <i class="fas fa-lock"></i>
                    <input type="password" class="form-control" id="password" name="password"
                           placeholder="Create a secure password" required minlength="6">
                </div>
                <div class="pw-hint"><i class="fas fa-info-circle" style="color:var(--s400);"></i>At least 6 characters</div>
            </div>
            <button type="submit" class="btn-save"><i class="fas fa-user-plus"></i>Create Account</button>
        </form>
        <div class="auth-footer-link">
            Already have an account? <a href="login.php"><i class="fas fa-sign-in-alt me-1"></i>Sign In</a>
        </div>
    </div>
</div>
...
document.getElementById('registrationForm').addEventListener('submit',function(e){
    ...
    if(!em.includes('@')||!em.includes('.')){e.preventDefault();alert('Please enter a valid email address.');return;}
